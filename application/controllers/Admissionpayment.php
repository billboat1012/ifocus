...
class Admissionpayment extends Frontend_Controller
{
    ...
    public function index($id = '')
    {
        ...
        if ($this->data['get_student']['status'] == 0) {
   ...
   public function verify_flutterwave_payment()
    {
        ...
        $params = isset($_COOKIE['payment_params']) ? json_decode($_COOKIE['payment_params'], true) : null;
        $this->session->set_userdata('params', $params);
    $config = $this->getPaymentConfig();


    setcookie('payment_params', '', time() - 3600, "/");
        ...
        $updateStatus = $this->db->update('online_admission', $arrayData);
        
        // Check if update was successful
        if (!$updateStatus) {
            log_message('error', $this->db->last_query() . ' Error: ' . $this->db->error()['message']);
            return false; // Handle the error appropriately
        }

        // transaction voucher save function
        $getSetting = $this->admissionpayment_model->get('transactions_links', array('branch_id' => $data['data']['branch_id']), true);
        if ($getSetting['status']) {
            $arrayTransaction = array(
                'account_id' => $getSetting['deposit'],
                'branch_id' => $getSetting['branch_id'],
                'amount' => $data['amount'],
                'date' => $data['date'],
            );
