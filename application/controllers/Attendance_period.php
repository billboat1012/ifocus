...
class Attendance_period extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->data['date'] = $date;
                $this->data['attendencelist'] = $this->attendance_period_model->getStudentAttendence($classID, $date, $subject_timetableID, $branchID);
            }
        }
    ...
    public function reportsbydate()
    {
        ...
        if ($_POST) {
            ...
            $this->data['subjectByClassSection'] = $this->attendance_period_model->getSubjectByClassSection($this->data['class_id'], $date);
            $this->data['studentlist'] = $this->attendance_model->getStudentList($branchID, $this->data['class_id']);
        }
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('student_attendance');
    ...
    public function reports()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->data['date'] = $date;
                $this->data['attendencelist'] = $this->attendance_period_model->getSubjectAttendanceReport($classID, $date, $subject_timetableID, $branchID);
            }
        }
        $this->data['getWeekends'] = $this->application_model->getWeekends($branchID);
        $this->data['getHolidays'] = $this->attendance_model->getHolidays($branchID);
    ...
    public function getByClassSection()
    {
        ...
        if (!empty($classID)) {
            $query = $this->attendance_period_model->getSubjectByClassSection($classID, $date);
            if ($query->num_rows() > 0) {
