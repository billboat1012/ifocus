...
class Timetable extends Admin_Controller
{
    ...
    public function viewexam()
    {
        ...
        if ($_POST) {
            $classID = $this->input->post('class_id');
            $sectionID = $this->input->post('section_id');
            $this->data['examlist'] = $this->timetable_model->getExamTimetableList($classID, $branchID);
        }
        $this->data['branch_id'] = $branchID;
    ...
    public function set_examwise()
    {
        ...
        if ($_POST) {
            ...
            $this->data['exam_type_id'] = $this->timetable_model->getExamType($examID);
            $this->data['class_id'] = $classID;
            $this->data['section_id'] = $sectionID;
            $this->data['subjectassign'] = $this->timetable_model->getSubjectExam($classID, $examID, $branchID);
        }
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('add') . " " . translate('schedule');
    ...
    public function exam_create()
    {
        ...
        if ($_POST) {
            ...
            $examTypeId = $this->timetable_model->getExamType($exam_id);
            $examMarks = $this->timetable_model->getExamMarks($exam_id);
            foreach ($items as $key => $value) {
                $this->form_validation->set_rules('timetable[' . $key . '][date]', translate('date'), 'required');
                $this->form_validation->set_rules('timetable[' . $key . '][time_start]', translate('starting_time'), 'required');
                $this->form_validation->set_rules('timetable[' . $key . '][time_end]', translate('ending_time'), 'required');
                $this->form_validation->set_rules('timetable[' . $key . '][hall_id]', translate('hall_room'), 'required');
                // callback_check_hallseat_capacity
		...
		{
		   $distribution = array();
		   foreach($examMarks as $id => $mark)
		   {
		       $distribution[$id]['full_mark'] = 10;
		   }
		   
		   foreach($examMarks as $id => $mark)
		   {
		       $distribution[$id]['pass_mark'] = 10;
		   }
		}else{
                    $distribution = array();
    ...
    public function exam_delete($examID, $classID)
    {
    ...
    public function getExamTimetableM()
    {
        ...
        $this->data['class_id'] = $classID;
        $this->data['section_id'] = $sectionID;
        $this->data['timetables'] = $this->timetable_model->getExamTimetableByModal($examID, $classID);
