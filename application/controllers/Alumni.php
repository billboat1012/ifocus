...
class Alumni extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->data['students'] = $this->alumni_model->getStudentListByClassSection($this->data['class_id'], $branchID, $this->data['passing_session']);
        }
    ...
    public function getEventDetails()
    {
        if (get_permission('alumni_events', 'is_view')) {
            ...
            $auditions = array("1" => "everybody", "2" => "class");
            if (!is_superadmin_loggedin()) {
                $this->db->where('branch_id', get_loggedin_branch_id());
            }
            $this->db->where('id', $id);
            ...
            if ($ev['audience'] != 1) {
                ...
                if ($ev['audience'] == 3) {
                    foreach ($selecteds as $selected) {
                        ...
                        $html .= "<br> <small> - " .  get_type_name_by_id('class', $selected[0]) . '</small>';
                    }
                }
            }
            $html .= "</td>";
