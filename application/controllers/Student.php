...
class Student extends Admin_Controller
{
    ...
    protected function student_validation($register_no = '', $student_id = '')
    {
        ...
        // Add 'student_id' as a parameter in set_rules to use in the callback function
        // Pass register_no and student_id as a JSON string to the validation rule
        $data = json_encode(['register_no' => $register_no, 'student_id' => $student_id]);
        $this->form_validation->set_rules('register_no', translate('register_no'), 'trim|required|callback_unique_registerid[' . $data . ']');
                ...
                if ($csv_array) {
                    ...
                    foreach ($csv_array as $row) {
                        ...
                        if (count($csv_chk) <= 0) {
                            ...
                            $r = $this->csvCheckExistsData($row['StudentUsername'], $row['Roll'], $row['RegisterNo'], $classID, $branchID, $unique_roll);
                            if ($r['status'] == false) {
                                $err_msg .= $row['FirstName'] . ' ' . $row['LastName'] . " - Imported Failed : " . $r['message'] . "<br>";
                            } else {
                                $this->student_model->csvImport($row, $classID, $branchID);
                                $i++;
                            }
                        } else {
    ...
    public function disable_authentication()
    {
        ...
        if (isset($_POST['search'])) {
            $classID = $this->input->post('class_id');
            $this->data['students'] = $this->application_model->getStudentListByClassSection($classID, $branchID, true);
        }
    ...
    public function view()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $this->data['students'] = $this->application_model->getStudentListByAcadYear($classID, $branchID, false, true, true, $sessionID);
        }
        $this->data['branch_id'] = $branchID;
    ...
    public function create_login()
    {
        ...
        if (isset($_POST['search'])) {
            $classID = $this->input->post('class_id');
            $sessionID = $this->input->post('session_id');
            $this->data['students'] = $this->application_model->getInactiveStudents($classID, $branchID, $sessionID, false, true);
        }
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('deactivated_student_list');
        $this->data['main_menu'] = 'student';
        $this->data['sub_page'] = 'student/create_login';
        $this->data['headerelements'] = array(
            'js' => array(
                'js/student.js'
            ),
    ...
    public function save_login_credentials()
{
    // Get the login data from the AJAX request
    $loginData = $this->input->post('login_data');
    ...
    if (!empty($loginData)) {
        foreach ($loginData as $data) {
            ...
            // Check if the username already exists in the login_credentials table
            $usernameExists = $this->db->get_where('login_credential', ['username' => $username])->row_array();
            if ($usernameExists) {
                // Respond with an error message if the username already exists
                echo json_encode(['success' => false, 'message' => 'Username already taken: ' . $username]);
                return; // Stop further processing
            }

            // Check if the password already exists for any student
            $passwordExists = $this->db->get_where('login_credential', ['password' => password_hash($password, PASSWORD_DEFAULT)])->row_array();
            if ($passwordExists) {
                // Respond with an error message if the password already exists
                echo json_encode(['success' => false, 'message' => 'Password already used for another student']);
                return; // Stop further processing
            }

            // If both username and password are unique, hash the password
            ...
            $insertData = [
                ...
                'role' => 7, // Role is always 7 for students
                'active' => 1, // Default active status
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
            ];

            // Insert the data into the login_credentials table
            $this->db->insert('login_credential', $insertData);
        }

        // Respond with success if no errors occurred
        echo json_encode(['success' => true]);
    } else {
        // Respond with an error if no login data was provided
    ...
    public function profile($id = '', $search = 'flse')
    {
        if($search == 'tUeR')
        {
            $tOrF = false;
        ...
        if (isset($_POST['update'])) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                elseif($tOrF == 'tUeR') :
                    redirect(base_url('student/profile/' . $id . $search));
                endif;
            }
        }
    ...
    public function csvCheckExistsData($student_username = '', $roll = '', $registerno = '', $class_id = '', $branchID = '', $unique_roll)
    {
        $array = array();
        if (!empty($roll)) {
    ...
    public function unique_registerid($json_data)
{
    // Decode the JSON string to get register_no and student_id
    ...
    // Exclude the current student ID if provided
    if ($student_id) {
        $this->db->where('id !=', $student_id);
    }
    
    // Check for uniqueness of register_no within the branch
    ...
    public function login_credential_reports()
    {
        ...
        if (isset($_POST['search'])) {
            $classID = $this->input->post('class_id');
            $this->data['students'] = $this->application_model->getStudentListByClassSection($classID, $branchID, false, true);
        }
        $this->data['branch_id'] = $branchID;
    ...
    public function admission_reports()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $this->data['end'] = $end;
            $this->data['students'] = $this->student_model->getStudentList($classID, $branchID, false, $start, $end)->result_array();
        }
        $this->data['branch_id'] = $branchID;
    ...
    public function classsection_reports()
    {
        ...
        $branchID = $this->application_model->get_branch_id();
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('class');
        $this->data['main_menu'] = 'student_repots';
    ...
    public function sibling_report()
    {
        ...
        if (isset($_POST['search'])) {
            $classID = $this->input->post('class_id');
            $getParentsList = $this->student_model->getParentList($classID, $branchID);
            $list = array();
            foreach ($getParentsList as $key => $parent) {
                if (intval($parent['child']) > 1) {
                    $getParentsList[$key]['student'] = $this->student_model->getSiblingListByClass($parent['parent_id'], $classID);
