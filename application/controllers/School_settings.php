...
class School_settings extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (isset($_FILES["stamp_file"]) && !empty($_FILES['stamp_file']['name'])) {
                    $fileInfo = pathinfo($_FILES["stamp_file"]["name"]);
                    $img_name = $id . '.' . $fileInfo['extension'];
                    move_uploaded_file($_FILES["stamp_file"]["tmp_name"], "uploads/app_image/stamp-logo-" . $img_name);
                }
                if (isset($_FILES["bg_file"]) && !empty($_FILES['bg_file']['name'])) {
                    $fileInfo = pathinfo($_FILES["bg_file"]["name"]);
                    $img_name = $id . '.' . $fileInfo['extension'];
                    move_uploaded_file($_FILES["bg_file"]["tmp_name"], "uploads/app_image/bg-logo-" . $img_name);
                }
    ...
    public function eSender()
    {
        ...
        if ($this->form_validation->run() !== false) {
            $arrayeSender = array(
                'field_one' => $this->input->post('api_key'),
            );
            $this->db->where('sms_api_id', 9);
            $this->db->where('branch_id', $branchID);
            $q = $this->db->get('sms_credential');
            if ($q->num_rows() == 0) {
                $arrayeSender['sms_api_id'] = 9;
                $arrayeSender['branch_id'] = $branchID;
                $this->db->insert('sms_credential', $arrayeSender);
            } else {
                $this->db->where('id', $q->row()->id);
                $this->db->update('sms_credential', $arrayeSender);
            }
            $message = translate('information_has_been_saved_successfully');
            $array = array('status' => 'success', 'message' => $message);
        } else {
