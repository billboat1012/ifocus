...

{
                ...
                if ($this->form_validation->run() !== false) {
                    ...
                    foreach ($subjects as $subject) {
                        ...
                        if ($query->num_rows() == 0) {
                            ...
                            // Optionally update incorrect assignments
                            $currentAssignment = $query->row();
                            if ($currentAssignment->teacher_id != (empty($get_teacher) ? 0 : $get_teacher['teacher_id'])) {
                                $this->db->where('id', $currentAssignment->id);
                                $this->db->update('subject_assign', [
                                    'teacher_id' => empty($get_teacher) ? 0 : $get_teacher['teacher_id'],
                                ]);
                            }
                        }
                    }
    ...
    public function class_assign_delete($class_id = '')
...

    {
        ...
        if ($q == 0) {
            ...
            $this->form_validation->set_message('unique_subject_assign', 'This class is already assigned.');
...
// add subject assign information and delete
...

    {
        ...
        if (!get_permission('sub_subject', 'is_view')) {
        ...
        $this->data['title'] = translate('sub_subject');

        $this->data['sub_page'] = 'subject/sub_subject';
...
public function add_sub_subject()

    {
        ...
        if ($_POST) {
            ...
            if (get_permission('sub_subject', 'is_add')) {


                $this->form_validation->set_rules('sub_subject', translate('subject'), 'trim|required');
                ...
                if ($this->form_validation->run() !== false) {
                    ...
                    $arraySubject = array(
                        ...
                        'sub_subject_text' => $this->input->post('sub_subject'),

                    );
                    $this->db->insert('sub_subject', $arraySubject);
...

    {
        ...
        if (!get_permission('sub_subject', 'is_delete')) {
...

    {
        ...
        if (!empty($classID)) {
            ...
            $query = $this->subject_model->getSubjectByClassSection($classID);
    ...
    public function getSubSubjects() {
        $subjectID = $this->input->post('subject_id');
        if ($subjectID) {
            $subSubjects = $this->exam_model->getSubSubjectId($subjectID);
