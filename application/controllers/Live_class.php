...
class Live_class extends Admin_Controller
{
                ...
                if ($this->form_validation->run() !== false) {
                    ...
                    if (isset($post['send_notification_sms'])) {
                        foreach ($post['section'] as $key => $value) {
                            $stuList = $this->application_model->getStudentListByClassSection($post['class_id'], $branchID);
    ...
    public function reports()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $this->data['livelist'] = $this->live_class_model->getReports($classID, $method, $start, $end, $branchID);
        }
        $this->data['headerelements'] = array(
            'css' => array(
