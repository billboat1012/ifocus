...
class Home extends Frontend_Controller
{
    ...
    public function admission()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                } elseif($getStudent['fee_elements']['status'] == 1) {
    ...
    {
        $this->load->model('admissionpayment_model');
        $getStudent = $this->admissionpayment_model->getStudentDetails_Two($studentID);
    ...
    if ($_POST) {
        try {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (empty($userID)) {
                    $error = "Register No Not Found.";
                    log_message('error', $error . ' Register No: ' . $registerNo);
                ...
                if (empty($templateID) || $templateID == 0) {
                    ...
                    log_message('error', $error . ' Template ID: ' . $templateID);
                    $array = array('status' => '0', 'error' => $error);
                    echo json_encode($array);
                    exit();
                }
                ...
                $htmlContent = $this->load->view('home/admitCardprintFn', $this->data, true);

                // Load your PDF library
                $this->load->library('Html2pdf');

                // Configure PDF library and generate PDF
                $pdf = new \Mpdf\Mpdf(); // For mPDF (adjust for HTML2PDF if needed)
                $pdf->SetTitle('Admit Card');
                $pdf->WriteHTML($htmlContent);
                $pdfOutput = 'Admit_Card_' . date('YmdHis') . '.pdf';

                // Output the PDF for download
                $pdf->Output($pdfOutput, 'D'); // 'D' for download
                exit(); // Stop further execution
            } else {
                // Handle validation errors
                $error = $this->form_validation->error_array();
                log_message('error', 'Validation Errors: ' . json_encode($error));
                $array = array('status' => 'fail', 'error' => $error);
                echo json_encode($array);
            }
        } catch (Exception $e) {
            // Log the exception
            log_message('error', 'Exception in admitCardprintFn: ' . $e->getMessage());
    ...
    public function examResultsPrintFn()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $this->db->select('student.id,enroll.class_id');
                $this->db->from('enroll');
                ...
                $result = $this->exam_model->getStudentReportCard($userID['id'], $examID, $sessionID, $userID['class_id']);
