...
class Ajax extends MY_Controller
{
    ...
    public function getSubjectByBranch()
    {
        ...
        $branch_id = $this->application_model->get_branch_id();
        if (!empty($branch_id)) {
            $subjects = $this->db->select('id,name')->where('branch_id', $branch_id)->get('subject')->result_array();
            if (count($subjects)) {
                $html .= "<option value=''>" . translate('select') . "</option>";
                foreach ($subjects as $row) {
                    $html .= '<option value="' . $row['id'] . '">' . $row['name'] . '</option>';
    ...
    public function getDistributionOptions()
    {
        ...
        if (!empty($branch_id)) {
            ...
            // Loop through results and create options array
            foreach ($result as $row) {
                $options[$row['id']] = $row['name'];
            }
        } else {
            $options = array("" => translate('select_branch_first'));
        }
    ...
    {
        $html = "";
        $html .= '<option value="">' . translate('not_required') . '</option>';
        echo $html;
    }
