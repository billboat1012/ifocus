...
class Student_promotion extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->data['students'] = $this->application_model->getStudentListByClassSection($this->data['class_id'], $this->data['session_id'], $branchID, false, true, false);
        }
    ...
    public function getPromotionStatus()
    {
        if ($_POST) {
            ...
            $r = $this->db->select('student_id')->where(array('class_id' => $class_id, 'session_id' => $session_id))->get('enroll')->result_array();
            if (empty($r) ) {
    ...
    function validSection($sectionID) {
            ...
            $promote_session_id = $this->input->post('promote_session_id');
            $promote_class_id = $this->input->post('promote_class_id');
            if (($promote_session_id == get_session_id()) && ($pre_class_id == $promote_class_id)) {
