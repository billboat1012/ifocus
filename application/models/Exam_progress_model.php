...
class Exam_progress_model extends CI_Model
{
    ...
    public function getExamTotalMark($studentID, $sessionID, $subjectID = '', $examID = '', $class_id = '')
    {
    ...
    public function getSubjectPosition($classID = '', $examID = [], $sessionID = '', $subjectID = '', $mark = 0)
    {
    ...
    public function getStudentReportCard($studentID = "", $sessionID = "", $class_id = "")
    {
        $result = array();
        $this->db->select('s.*,CONCAT_WS(" ",s.first_name, s.last_name) as name,e.id as enrollID,e.roll,e.branch_id,e.session_id,e.class_id,c.name as class, sc.name as category,IFNULL(p.father_name,"N/A") as father_name,IFNULL(p.mother_name,"N/A") as mother_name,br.name as institute_name,br.email as institute_email,br.address as institute_address,br.mobileno as institute_mobile_no');
