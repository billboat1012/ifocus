...
class Fees_model extends MY_Model
{
    ...
    public function getStudentAllocationList($classID = '', $groupID = '', $branchID = '')
    {
    ...
    public function getInvoiceBasic($enrollID = '')
    {
        ...
        $this->db->select('s.id,s.register_no,e.branch_id,e.id as enroll_id,s.first_name,s.last_name,s.email as student_email,s.current_address as student_address,c.name as class_name,b.school_name,b.email as school_email,b.mobileno as school_mobileno,b.address as school_address,p.father_name');
    ...
    public function getInvoiceList($class_id = '', $branch_id = '')
    {
        $this->db->select('e.id as enroll_id,e.student_id,e.roll,e.id as enroll_id,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name');
    ...
    public function getDueInvoiceList($class_id = '', $feegroup_id = '', $fee_feetype_id = '')
    {
        $sql = "SELECT IFNULL(SUM(h.amount), '0') as `total_amount`, IFNULL(SUM(h.discount), '0') as `total_discount`, gd.amount as `full_amount`, fa.prev_due as `prev_due`, gd.`due_date`, e.student_id, e.id as enroll_id,e.roll, s.first_name, s.last_name, s.register_no, s.mobileno, c.name as `class_name` FROM fee_allocation as fa LEFT JOIN fee_payment_history as h ON h.allocation_id = fa.id and h.type_id = " .
        ...
        enroll as e ON e.id = fa.student_id LEFT JOIN student as s ON s.id = e.student_id LEFT JOIN class as c ON c.id = e.class_id WHERE
    ...
    public function getDueReport($class_id = '')
    {
        $this->db->select('fa.id as allocation_id,sum(gd.amount + fa.prev_due) as total_fees,e.id as enroll_id,e.roll,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name');
        $this->db->from('fee_allocation as fa');
    ...
    public function getStuPaymentHistory($classID = '', $paymentVia = '', $start = '', $end = '', $branchID = '', $onlyFine = false)
    {
        $this->db->select('h.*,ft.name as type_name,e.student_id,e.roll,s.first_name,s.last_name,s.register_no,s.mobileno,c.name as class_name,pt.name as pay_via');
        $this->db->from('fee_payment_history as h');
    ...
    public function getStuPaymentReport($classID = '', $enrollID = '', $typeID = '', $start = '', $end = '', $branchID = '')
    {
