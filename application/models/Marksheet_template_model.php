...

{
    ...
    {
        $this->db->select('marksheet_template.id, marksheet_template.name, branch.name as branchname');
...

    {
        ...
        $arrayLive = array(
            ...
            'stamp' => $this->fileupload('stamp_file', './uploads/marksheet/', $this->input->post('old_stamp_file')),
            ...
            'playgroup' => isset($_POST['playgroup']) ? 1 : 0,
...

    {
        ...
        foreach ($tags as $tag) {
            ...
            if ($field == 'student_photo') {
                ...
                $photo = '<img height="70" class="' . ($photo_style == 1 ? '' : 'rounded') . '" src="' . get_image_url('student', $userDetails['photo']) . '">';
                ...
                if (!empty($template['stamp'])) {

                    $signature_ph = '<img src="' . base_url('uploads/marksheet/' . $template['stamp']) . '">';
                ...
                if (!empty($studentData['term'])) {

                    $body = str_replace($tag, $studentData['term'], $body);
            ...
            } else if($field == 'resumption_date') {
                
                if(!empty($studentData['resumption_date'])){
                    $res_date = $this->application_model->formatNiceDate($studentData['resumption_date']);
                    $body = str_replace($tag, $res_date, $body);
                } else {
                    $body = str_replace($tag, '28, July, 2025', $body);
                }
            
            } else if ($field == 'principal_comments') {
                $comment = !empty($extendsData['principal_comments']) ? $extendsData['principal_comments'] : "";
                $body = str_replace($tag, $comment, $body);
            } else if ($field == 'teacher_comments') {
                $comment = !empty($extendsData['teacher_comments']) ? $extendsData['teacher_comments'] : "";
                $body = str_replace($tag, $comment, $body);
            } else if ($field == 'hostel_comment') {
                $comment = !empty($extendsData['hostel_comment']) ? $extendsData['hostel_comment'] : "";
                $body = str_replace($tag, $comment, $body);
            } else {
    ...
    public function getExamTerm($id)
    {
    ...
    public function getExamTermFor3rdTerm($id)
    {
        $column = 'slug';
