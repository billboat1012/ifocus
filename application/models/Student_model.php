...
class Student_model extends MY_Model
{
    ...
    public function csvImport($row = array(), $classID = '', $branchID = '')
    {
    ...
    public function getStudentList($classID = '',  $branchID = '', $deactivate = false, $start = '', $end = '')
    {
        $this->db->select('e.*,s.photo, CONCAT_WS(" ", s.first_name, s.last_name) as fullname,s.register_no,s.gender,s.admission_date,s.parent_id,s.email,s.blood_group,s.birthday,l.active,c.name as class_name');
    ...
    {
        $this->db->select('e.*,s.photo,s.first_name,s.last_name,s.register_no,s.parent_id,s.email,s.blood_group,s.birthday,c.name as class_name,sp.name as parent_name');
        $this->db->from('enroll as e');
    ...
    {
        $this->db->select('s.*, l.username, l.active, e.class_id, e.branch_id, e.session_id, sc.name as category_name, c.name as class_name, e.id as enrollid, e.roll as roll');
        $this->db->from('enroll as e');
        $this->db->join('student as s', 'e.student_id = s.id', 'left');
        $this->db->join('login_credential as l', 'l.user_id = s.id and l.role = 7', 'left'); // Changed to LEFT JOIN
        $this->db->join('class as c', 'e.class_id = c.id', 'left');
    ...
    {
        $this->db->select('s.photo, s.register_no, CONCAT_WS(" ",s.first_name, s.last_name) as fullname,s.gender,s.mobileno,e.roll,e.branch_id,c.name as class_name');
        $this->db->from('enroll as e');
        $this->db->join('student as s', 'e.student_id = s.id', 'inner');
        $this->db->join('class as c', 'e.class_id = c.id', 'left');
    ...
    public function getParentList($class_id = '',  $branch_id = '')
    {
    ...
    public function getSiblingListByClass($parent_id = '', $class_id = '')
    {
        $this->db->select('s.register_no,e.id as enroll_id,CONCAT_WS(" ",s.first_name, s.last_name) as fullname,s.gender,c.name as class_name');
        $this->db->from('enroll as e');
        $this->db->join('student as s', 'e.student_id = s.id', 'inner');
        $this->db->join('class as c', 'e.class_id = c.id', 'left');
        $this->db->where('e.class_id', $class_id);
