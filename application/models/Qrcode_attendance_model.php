...
class Qrcode_attendance_model extends MY_Model
{
    ...
    {
        $this->db->select('s.first_name,s.last_name,s.register_no,s.email,s.photo,admission_date,birthday,enroll.student_id,enroll.branch_id,enroll.roll,class.name as class_name,student_category.name as cname');
        $this->db->from('enroll');
    ...
    public function getStuListDT($postData)
    {
        ...
        // Fetch records
        $sql = "SELECT `e`.`id` as `enroll_id`, `e`.`roll`, `s`.`register_no`, CONCAT_WS(' ', `s`.`first_name`, `s`.`last_name`) as `fullname`, `class`.`name` as `class_name`, `s`.`register_no`, `st`.`date`, `st`.`in_time`, `st`.`out_time` FROM `student_attendance` as `st` INNER JOIN `enroll` as `e` ON `e`.`id` = `st`.`enroll_id` INNER JOIN `student` as `s` ON `s`.`id` = `e`.`student_id` LEFT JOIN `class` ON `class`.`id` = `e`.`class_id` WHERE `e`.`session_id` = $sessionID AND `st`.`qr_code` = '1' AND date(st.date) = $today";
        if (!empty($searchQuery)) {
            $sql .= " AND " . $searchQuery;
        }
    ...
    public function getDailyStudentReport($branchID = '', $classID = '', $date = '')
    {
