...
class Alumni_model extends MY_Model
{
    ...
    public function getStudentListByClassSection($classID = '',   $branchID = '', $session_id ='')
    {
        $sql = "SELECT `e`.`id`, CONCAT_WS(' ',`s`.`first_name`, `s`.`last_name`) as `fullname`, `s`.`register_no`, `s`.`gender`,`alumni`.`id` as `alumni_id`, `alumni`.`email`, `alumni`.`mobile_no`,`alumni`.`address`,`alumni`.`profession`,`alumni`.`photo`,`c`.`name` as `class_name` FROM `enroll` as `e` INNER JOIN `student` as `s` ON `e`.`student_id` = `s`.`id` LEFT JOIN `class` as `c` ON `e`.`class_id` = `c`.`id` LEFT JOIN `alumni_students` as `alumni` ON `alumni`.`enroll_id` = `e`.`id` WHERE `e`.`class_id` = " . $this->db->escape($classID) . " AND `e`.`branch_id` = " . $this->db->escape($branchID) . " AND `e`.`session_id` = " . $this->db->escape($session_id);
        $sql .= " AND `e`.`is_alumni` = '1'  ORDER BY `s`.`id` ASC";
        
        return $this->db->query($sql)->result_array();
    }

    public function getList($branchID = '', $classID = '',    $session_id = '')
    {
