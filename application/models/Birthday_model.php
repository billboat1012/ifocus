...
class Birthday_model extends MY_Model
{
    ...
    // {
    //     $sql = "SELECT `e`.*, `s`.`photo`, CONCAT_WS(' ',`s`.`first_name`, `s`.`last_name`) as `fullname`, `s`.`register_no`, `s`.`parent_id`, `s`.`email`, `s`.`mobileno`, `s`.`birthday`, `c`.`name` as `class_name` FROM `enroll` as `e` INNER JOIN `student` as `s` ON `e`.`student_id` = `s`.`id` LEFT JOIN `class` as `c` ON `e`.`class_id` = `c`.`id` WHERE `e`.`session_id` = " . $this->db->escape(get_session_id());
    //     if (!empty($start)) {
    ...
    public function getStudentListByBirthday($branchID = '', $start = '', $end = '')
    {
        $sql = "SELECT `e`.*, 
                ...
                WHERE `e`.`session_id` = " . $this->db->escape(get_session_id());
    
        if (!empty($start) && !empty($end)) {
            $startFormatted = date('m-d', strtotime($start));
            $endFormatted = date('m-d', strtotime($end));
            $sql .= " AND DATE_FORMAT(`s`.`birthday`, '%m-%d') BETWEEN " . $this->db->escape($startFormatted) . " AND " . $this->db->escape($endFormatted);
        }
