...
class Admissionpayment_model extends MY_Model
{
    ...
    public function getStudentDetails($referenceNo)
    {
        $amount = 0;
        $status = 0;
        $this->db->select('online_admission.*,branch.name as branch_name,branch.symbol,branch.currency,class.name as class_name,front_cms_admission.fee_elements');
    ...
    public function getStudentDetails_Two($referenceNo)
    {
        $amount = 0;
        $status = 0;
        $this->db->select('online_admission.*,branch.name as branch_name,branch.symbol,branch.currency,class.name as class_name,front_cms_admission.fee_elements');
        ...
        $this->db->join('class', 'class.id = online_admission.class_id', 'left');
        $this->db->join('front_cms_admission', 'front_cms_admission.branch_id = online_admission.branch_id', 'left');
        $this->db->where('online_admission.id', $referenceNo);
        $q = $this->db->get()->row_array();
        $classID = $q['class_id'];
        $elements = json_decode($q['fee_elements'], true);
        if (isset($elements[$classID]) && !empty($elements[$classID])) {
