...
class Onlineexam_model extends MY_Model
{
    ...
    public function saveQuestions()
    {
        ...
        if (!empty($classID)) {
            $unique_id = $this->db->select('parent_class')
                ->from('class')
                ->where('id', $classID) // Use $classID instead of $class_id
                ->get()
                ->row('parent_class');
        
            $questionsExam['class_parent_id'] = $unique_id;
    ...
    public function questionList($postData)
    {
        ...
        $query = $this->db->query("SELECT parent_class FROM class WHERE id = ?", array($classID));
        $result = $query->row();
        $classParentID = ($result && !empty($result->parent_class)) ? $result->parent_class : '';
        ...
        // if($classParentID != ''){
        //     $classParentID = $this->db->escape($classParentID);
        //     $search_arr[] = " `questions`.`class_parent_id` = $classParentID ";
        // }
    ...
    public function getExamResults($onlineexamID = null, $studentID = 0)
    {
        $sql = "SELECT DISTINCT `questions_manage`.*, 
                       `questions`.`id` as `qus_id`, 
                       `questions`.*, 
                       COALESCE(`online_exam_answer`.`answer`, '') as `sb_ans`
                FROM `questions_manage`
                INNER JOIN `questions` 
                    ON `questions`.`id` = `questions_manage`.`question_id`
                ...
                WHERE `questions_manage`.`onlineexam_id` = " . $this->db->escape($onlineexamID) . "
                GROUP BY `questions_manage`.`question_id`
    ...
    public function examReport($examID = '', $classID = '', $branchID = '', $order_by_position = 0)
{
    $this->db->select('online_exam_submitted.student_id, MAX(online_exam_submitted.id) as latest_submission_id, online_exam_submitted.remark, online_exam_submitted.position, online_exam.id, online_exam.neg_mark, online_exam.mark_type, online_exam.passing_mark, student.first_name, student.last_name, student.register_no, student.mobileno, online_exam.title, online_exam.subject_id, class.name as class_name');
    $this->db->from('online_exam_submitted');
    $this->db->join('online_exam', 'online_exam.id = online_exam_submitted.online_exam_id', 'inner');
    $this->db->join('class', 'class.id = online_exam.class_id', 'left');
    $this->db->join('student', 'student.id = online_exam_submitted.student_id', 'left');
    ...
    $this->db->where('online_exam.session_id', get_session_id());
    $this->db->where('online_exam.class_id', $classID);
    $this->db->group_by('online_exam_submitted.student_id');
    $result = $this->db->get()->result_array();
    ...
    foreach ($result as $key => $value) {
        ...
        if ($examResult['total_obtain_marks'] != 0) {
            if ($value['mark_type'] == 1) {
                $obtain = ((($examResult['total_obtain_marks'] - $total_neg_marks) * 100) / $examResult['total_marks']);
                $status = ($obtain >= $value['passing_mark']) ? 1 : 0;
            } else {
                $obtain = ($examResult['total_obtain_marks'] - $total_neg_marks);
                $status = ($obtain >= $value['passing_mark']) ? 1 : 0;
            }
    ...
    public function examResult($examID, $studentID)
    {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $key => $value) {
                ...
                // Ensure `sb_ans` is not null or empty before processing
                if (isset($value->sb_ans) && trim($value->sb_ans) !== '') {
                    $total_answered++;
    
                    if ($value->type == 1 || $value->type == 3) { // Single Choice / True-False
                        if ($value->sb_ans == $value->answer) {
                            $correct_ans++;
                            $total_obtain_marks += $value->marks;
                        } else {
                            $total_neg_marks += $value->neg_marks;
                            $wrong_ans++;
                        }
                    } elseif ($value->type == 2) { // Multiple Choice
                        // Ensure `json_decode` is always an array
                        $correctAnswer = json_decode($value->answer, true) ?: [];
                        $studentAnswer = json_decode($value->sb_ans, true) ?: [];
    
                        if ($this->array_equal($correctAnswer, $studentAnswer)) {
                            $correct_ans++;
                            $total_obtain_marks += $value->marks;
                        } else {
                            $total_neg_marks += $value->neg_marks;
                    ...
                    } elseif ($value->type == 4) { // Fill-in-the-Blanks
                        // Ensure spaces are normalized
                        $correctAns = preg_replace('/\s+/', '_', trim(strtolower($value->answer)));
                        $studentAns = preg_replace('/\s+/', '_', trim(strtolower($value->sb_ans)));
    ...
    public function getStudentDetailedResult($exam_id) {
        $this->db->select('exam.name as exam_name, exam.date, exam.duration, result.total_marks, result.obtain_marks, result.percentage, result.status, result.answers');
        $this->db->from('exam_results result');
        $this->db->join('exams exam', 'exam.id = result.exam_id');
        $this->db->where('result.exam_id', $exam_id);
        $this->db->where('result.student_id', $this->session->userdata('student_id'));
        $query = $this->db->get();
    ...
    public function getStudentAnswers($examID, $studentID) {
        ...
        $this->db->where(['online_exam_id' => $examID, 'student_id' => $studentID]);
        $query = $this->db->get('online_exam_answer');
