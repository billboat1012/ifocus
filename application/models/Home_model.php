...
class Home_model extends MY_Model
{
    ...
    public function getExamList($branchID = '', $classID = '')
    {
    ...
    {
        $result = 0;
        if ($type == 'class' || $type == 'live_class' || $type == 'subject' || $type == 'exam' || $type == 'book' || $type == 'branch') {
            $this->db->select('id');
            $this->db->from($type);
            if ($type != 'branch') {
                $this->db->where("branch_id", $branch_id);
    ...
    public function getPopup($branchID='')
    {
        $this->db->limit(1);
        $this->db->where('show_web', 1);
        $this->db->where('branch_id', $branchID);
        $this->db->order_by("id", "desc");
        $popup_list = $this->db->get('front_cms_popup_list')->result();
        return $popup_list;
    }
