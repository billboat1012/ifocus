...
class Application_model extends CI_Model
{
    ...
    public function getStudentListByClassSection($classID = '', $branchID = '', $deactivate = false, $rollOrder = false, $is_alumni = true, $sessionID = '')
    {
        $sql = "SELECT `e`.*, `s`.`photo`, CONCAT_WS(' ',`s`.`first_name`, `s`.`last_name`) as `fullname`, 
                       `s`.`register_no`, `s`.`gender`, `s`.`parent_id`, `s`.`email`, `s`.`mobileno`, `s`.`blood_group`, 
                       `s`.`birthday`, `s`.`admission_date`, `l`.`active`, `l`.`username` as `stu_username`, 
                ...
                LEFT JOIN `student_category` as `sc` ON `sc`.`id` = `s`.`category_id` 
                WHERE `e`.`class_id` = " . $this->db->escape($classID) . " 
                      AND `e`.`branch_id` = " . $this->db->escape($branchID);
        
        if (!empty($sessionID)) {
            $sql .= " AND `e`.`session_id` = " . $this->db->escape($sessionID);
        }
    ...
    public function getStudentListByAcadYear($classID = '', $branchID = '', $deactivate = false, $rollOrder = false, $is_alumni = true, $sessionID = '')
{
    $sql = "SELECT 
                `e`.*, 
                `s`.`photo`, 
                ...
                `s`.`birthday`, 
                `s`.`admission_date`, 
                IFNULL(`l`.`active`, 0) as `active`, 
                IFNULL(`l`.`username`, 'N/A') as `stu_username`, 
                `c`.`name` as `class_name`, 
                `sc`.`name` as `category` 
            FROM 
                `enroll` as `e` 
            INNER JOIN 
                `student` as `s` ON `e`.`student_id` = `s`.`id` 
            LEFT JOIN 
                `login_credential` as `l` ON `l`.`user_id` = `s`.`id` AND `l`.`role` = 7 
            LEFT JOIN 
                `class` as `c` ON `e`.`class_id` = `c`.`id` 
            LEFT JOIN 
                `student_category` as `sc` ON `sc`.`id` = `s`.`category_id` 
            WHERE 
                `e`.`class_id` = " . $this->db->escape($classID) . " 
                AND `e`.`branch_id` = " . $this->db->escape($branchID);
    ...
    if ($deactivate) {
        $sql .= " AND (`l`.`active` = 0 OR `l`.`active` IS NULL)";
    }
    ...
    public function getInactiveStudents($classID = '', $branchID = '', $sessionID = '')
    {
        $sql = "SELECT 
                    `e`.*, 
                    `e`.`id` as `enroll_id`, 
    ...
    {
        $this->db->select('s.*,e.class_id,e.id as enrollid,e.roll,e.branch_id,e.session_id,c.name as class_name,sc.name as category_name');
    ...
    {
        $this->db->select('s.last_name, e.class_id, c.name');
        $this->db->from('student as s');
        $this->db->join('enroll as e', 'e.student_id = s.id');
