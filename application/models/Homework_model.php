...
class Homework_model extends MY_Model
{
    ...
    public function getList($classID, $subjectID, $branchID)
    {
        $this->db->select('homework.*,subject.name as subject_name,class.name as class_name,staff.name as creator_name');
    ...
    public function evaluationCounter($classID, $homeworkID)
    {
        $countStu = $this->db->where(array('class_id' => $classID, 'session_id' => get_session_id()))->get('enroll')->num_rows();
    ...
    {
        $this->db->select('homework.*,CONCAT_WS(" ",s.first_name, s.last_name) as fullname,s.register_no,e.student_id, e.roll,subject.name as subject_name,class.name as class_name,he.id as ev_id,he.status as ev_status,he.remark as ev_remarks,he.rank,hs.message,hs.enc_name');
        $this->db->from('homework');
    ...
    public function save($data)
    {
        ...
        if (isset($data['notification_sms'])) {
        	$stuList = $this->application_model->getStudentListByClassSection($arrayHomework['class_id'], $arrayHomework['branch_id']);
