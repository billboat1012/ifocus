...
class Card_manage_model extends MY_Model
{
    ...
    public function tagsReplace($roleID, $userID, $templete, $print_date, $expiry_date)
    {
        ...
        return $body;
            ...
            // Log the exception
            log_message('error', 'Exception in tagsReplace: ' . $e->getMessage());
    ...
    public function admitCardTagsReplace($userID, $template, $print_date, $exam_id)
{
    try {
        ...
        // Default placeholder for missing or null values
        $default_placeholder = '---';

        foreach ($tags as $tag) {
            $field = str_replace(array('{', '}'), '', $tag);

            switch ($field) {
                case 'student_photo':
                    $photo = !empty($userDetails['photo']) 
                        ? '<img class="' . ($photo_style == 1 ? '' : 'rounded') . '" src="' . get_image_url('student', $userDetails['photo']) . '" width="' . $photo_size . '">'
                        : $default_placeholder;
                    $body = str_replace($tag, $photo, $body);
                    break;

                case 'exam_name':
                    $examName = $this->application_model->exam_name_by_id($exam_id) ?? $default_placeholder;
                    $body = str_replace($tag, $examName, $body);
                    break;

                case 'subject_list_table':
                    $subjectTable = $this->tableHtml($exam_id, $userDetails['class_id'] ?? 0, $userDetails['branch_id'] ?? 0) ?? $default_placeholder;
                    $body = str_replace($tag, $subjectTable, $body);
                    break;

                case 'logo':
                    $logo_ph = !empty($template['logo']) 
                        ? '<img src="' . base_url('uploads/certificate/' . $template['logo']) . '">'
                        : $default_placeholder;
                    $body = str_replace($tag, $logo_ph, $body);
                    break;

                case 'signature':
                    $signature_ph = !empty($template['signature']) 
                        ? '<img src="' . base_url('uploads/certificate/' . $template['signature']) . '">'
                        : $default_placeholder;
                    $body = str_replace($tag, $signature_ph, $body);
                    break;

                case 'qr_code':
                    if (!empty($template['qr_code'])) {
                        $qr_code = $template['qr_code'];
                        $params = [
                            'savename' => 'uploads/qr_code/stu_' . ($userDetails['id'] ?? 'unknown') . '.png',
                            'level' => 'M',
                            'size' => 2,
                            'data' => ucfirst($qr_code) . " - " . ($userDetails[$qr_code] ?? $default_placeholder)
                        ];
                        $qrCode = $this->ciqrcode->generate($params);
                        $photo = '<img src="' . base_url($qrCode) . '">';
                        $body = str_replace($tag, $photo, $body);
                    } else {
                        $body = str_replace($tag, $default_placeholder, $body);
                    }
                    break;

                case 'present_address':
                    $presentAddress = $userDetails['current_address'] ?? $default_placeholder;
                    $body = str_replace($tag, $presentAddress, $body);
                    break;

                case 'print_date':
                    $printDate = _d($print_date) ?? $default_placeholder;
                    $body = str_replace($tag, $printDate, $body);
                    break;

                default:
                    $value = $userDetails[$field] ?? $default_placeholder;
        ...
        log_message('error', 'Error in admitCardTagsReplace: ' . $e->getMessage());
        $response = [
            'status' => 'error',
...
{
    try {
        // Select the necessary fields from the student and enroll tables
        $this->db->select('s.*,CONCAT_WS(" ",s.first_name, s.last_name) as name,e.roll,e.id as attendance,e.class_id,e.branch_id,e.session_id,c.name as class,sc.name as category,p.father_name,p.mother_name,br.name as institute_name,br.email as institute_email,br.address as institute_address,br.mobileno as institute_mobile_no');
        ...
        if ($query->num_rows() === 0) {
            throw new Exception("No student found for enroll ID {$id}");
        }

        return $query->row_array();
    } catch (Exception $e) {
        // Log the error message
        log_message('error', 'Error in getStudent: ' . $e->getMessage());

        // Optionally, throw a new exception to be handled by the caller
    ...
    public function tableHtml($examID, $classID, $branchID = '')
    {
        $html = '';
        ...
        $html .= '</thead>';
        $html .= '<tbody>';
        $timetables = $this->timetable_model->getExamTimetableByModal($examID, $classID, $branchID);
