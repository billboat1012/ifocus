    ...
    function getClassTeacher($classID = '')
{
    if (loggedin_role_id() == 3) {
        ...
        if ($getMode == 0) {
            ...
            // Step 1: Fetch classes from teacher_allocation
            $this->CI->db->select('class.id as id, class.name as name');
            ...
            $allocatedClasses = $this->CI->db->get()->result_array();

            // Step 2: Fetch additional classes from subject_assign
            $this->CI->db->select('class.id as id, class.name as name');
            $this->CI->db->from('subject_assign');
            $this->CI->db->join('class', 'class.id = subject_assign.class_id', 'left');
            $this->CI->db->where('subject_assign.teacher_id', get_loggedin_user_id());
            $this->CI->db->where('subject_assign.session_id', get_session_id());

            // Exclude already allocated class IDs
            $allocatedClassIDs = array_column($allocatedClasses, 'id');
            if (!empty($allocatedClassIDs)) {
                $this->CI->db->where_not_in('class.id', $allocatedClassIDs);
            }

            $additionalClasses = $this->CI->db->get()->result_array();

            // Step 3: Combine and deduplicate
            $allClasses = array_merge($allocatedClasses, $additionalClasses);
            $allClasses = array_map("unserialize", array_unique(array_map("serialize", $allClasses)));

            return $allClasses; // Returns classes with `id` and `name` keys
        }
    } else {
        return false;
    }
