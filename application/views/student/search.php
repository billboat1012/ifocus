<?php if (!empty($query)):?>
				...
				<tbody>
					...
					<?php } ?>
						...
						<?php if (get_permission('student', 'is_edit')): ?>
							<!-- update link -->
							<a href="<?php echo base_url('student/profile/' . $row->student_id) . '/tUeR';?>" class="btn btn-default icon btn-circle" data-toggle="tooltip" data-original-title="<?=translate('details')?>">
	...
	<?php endif;?>
	<script>
        var baseUrl = "<?php echo base_url('student/profile/') ?>";
    </script>
    <script>
        $(".redirectBtn").click(function (e) {
            var value = $(this).val(); // Assuming the button has a `value` attribute
            window.location.href = baseUrl + value; // Redirect to the constructed URL
        });
    </script>
