						       	...
						       	if ($question->type == 1) {
						       	 ...
						       	 ?>
								       <div class="radio-custom radio-success mt-md">
								          <?php $savedAnswer = isset($previousAnswers[$question->question_id]) ? $previousAnswers[$question->question_id] : null; ?>
<input type="radio" value="<?=$quesOption_value?>" name="answer[<?=$question->question_id?>][<?=$question->type?>]" id="opt<?=$key . $quesOption_value?>" <?= ($savedAnswer == $quesOption_value) ? 'checked' : '' ?>>
									...
									?>
										<div class="checkbox-replace mt-lg">
											<label class="i-checks"><?php 
$savedAnswers = isset($previousAnswers[$question->question_id]) ? json_decode($previousAnswers[$question->question_id], true) : [];
?>
<input type="checkbox" name="answer[<?=$question->question_id?>][<?=$question->type?>][]" value="<?=$quesOption_value?>" <?= in_array($quesOption_value, $savedAnswers) ? 'checked' : '' ?>>
<i></i><?=$question->{$quesOption_key}?></label>
										</div>
						          <?php } } } elseif ($question->type == 3) { ?>
						          <?php $savedAnswer = isset($previousAnswers[$question->question_id]) ? $previousAnswers[$question->question_id] : null; ?>
								       <div class="radio-custom radio-success mt-md">
								           <input type="radio" value="1" name="answer[<?=$question->question_id?>][<?=$question->type?>]" id="tf1<?=$key?>" <?= ($savedAnswer == "1") ? 'checked' : '' ?>>
								           <label for="tf1<?=$key?>">TRUE</label>
								       </div>
								       <div class="radio-custom radio-success mt-md">
								           <input type="radio" value="0" name="answer[<?=$question->question_id?>][<?=$question->type?>]" id="tf0<?=$key?>" <?= ($savedAnswer == "0") ? 'checked' : '' ?>>
								           <label for="tf0<?=$key?>">FALSE</label>
								       </div>
						          <?php } elseif ($question->type == 4) { ?>
	                            <div class="form-group">
	                              <label class="control-label">Answer</label>
	                              <?php $savedAnswer = isset($previousAnswers[$question->question_id]) ? $previousAnswers[$question->question_id] : ''; ?>
                                        <input type="text" class="form-control" name="answer[<?=$question->question_id?>][<?=$question->type?>]" value="<?=$savedAnswer?>">
	                            </div>
						         <?php } ?>
