...
if (empty($studentSubmitted)) {
    if (($startTime <= $now && $now <= $endTime) && $exam->publish_status == 1) {
        ...
        $myTakes = $this->onlineexam_model->getStudentAttempt($exam->id);
        if ($myTakes > 0) {
            echo '<button class="btn btn-default btn-lg mt-lg start_btn" data-examid="' . $exam->id . '" data-loading-text="<i class=\'fas fa-spinner fa-spin\'></i> Processing">
                    <i class="fas fa-computer-mouse"></i> ' . translate('retake_exam') . '
                  </button>';
        } else {
            echo '<button class="btn btn-default btn-lg mt-lg start_btn" data-examid="' . $exam->id . '" data-loading-text="<i class=\'fas fa-spinner fa-spin\'></i> Processing">
    ...
    echo '<div class="text-center">
            <button class="btn btn-default btn-lg mt-lg start_btn" data-examid="' . $exam->id . '" data-loading-text="<i class=\'fas fa-spinner fa-spin\'></i> Processing">
                <i class="fas fa-computer-mouse"></i> ' . translate('retake_exam') . '
            </button>
          </div>';
} else {
...
<script type="text/javascript">
	...
	function savePartialAnswer(questionID) {
        var formData = $('#answerForm').serialize(); // Get all answers
    
        $.ajax({
            type: 'POST',
            url: base_url + "userrole/onlineexam_partial_save",
            data: formData,
            dataType: 'JSON',
            success: function(response) {
                console.log("Temporary save successful for question " + questionID);
            },
            error: function(xhr) {
    ...
    //     savePartialAnswer(currentStep);
    // }, 120000); 
	
	// Modify changeQuestion to auto-save before switching questions
    function changeQuestion(questionID) {
        // savePartialAnswer(currentStep); // Save before moving to next question
