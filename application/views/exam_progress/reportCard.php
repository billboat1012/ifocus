...
if (!empty($student_array)) {
	foreach ($student_array as $sc => $studentID) {
		$result = $this->exam_progress_model->getStudentReportCard($studentID, $sessionID, $class_id);
...
<?php } ?>
			...
			$grand_full_marks = 0;
			$result_status = 1;
			$getSubjectsList = $this->subject_model->getSubjectByClassSection($student['class_id']);
			...
			foreach ($getSubjectsList as $row) {
				...
				<tr>
					...
					$getExamTotalMark = $this->exam_progress_model->getExamTotalMark($studentID, $sessionID, $row['subject_id'], $id, $student['class_id']);
...
<?php } if ($marksheet_template['subject_position'] == 1) {?>
					<td valign="middle"><?php echo $this->exam_progress_model->getSubjectPosition($student['class_id'],  $examArray, $sessionID, $row['subject_id'], $subTotalObtain); ?></td>
<?php } ?>
				</tr>
			<?php } ?>
