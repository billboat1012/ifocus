...
$getTotalWorking = $this->db->select('total_days')
                            ->where('student_id', $studentID)
                            ->where('branch_id', $getExam['branch_id'])
                            ->where('class_id', $class_id)
                            ->get('attendance_update')
                            ->row();

$getTotalAttendance = $this->db->select('days_present')
                               ->where('student_id', $studentID)
                               ->where('branch_id', $getExam['branch_id'])
                               ->where('class_id', $class_id)
                               ->get('attendance_update')
...
$totalDays = $getTotalWorking->total_days ?? 0;
$daysPresent = $getTotalAttendance->days_present ?? 0;

$attenPercentage = $totalDays > 0 ? ($daysPresent / $totalDays) * 100 : 0.00;
?>

<table class="table table-bordered table-condensed">
    <tbody>
        <tr>
            <th colspan="2" style="background-color: #0F335C;text-align: center;color: #fff;">Attendance</th>
        </tr>
        <tr>
