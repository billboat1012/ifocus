				...
				<table class="table table-bordered table-hover table-condensed table-export mt-md">
					...
					<?php $count = 1; foreach($examlist as $row): ?>
						<tr>
							...
							<td>
								<!-- view link -->
								<a href="javascript:void(0);" class="btn btn-circle btn-default icon" onclick="getExamTimetableM('<?=$row['exam_id']?>','<?=$row['class_id']?>');"> 
									<i class="far fa-eye"></i> 
								</a>

								<?php if (get_permission('exam_timetable', 'is_delete')): ?>
								<!-- delete link -->
								<?php echo btn_delete('timetable/exam_delete/' . $row['exam_id'] .'/'.  $row['class_id']);?>
								<?php endif; ?>
							</td>
						</tr>
					<?php endforeach; ?>
