		...
		<div class="tab-content">
			<div id="edit" class="tab-pane active">
				...
				<input type="hidden" name="subject_id" value="<?=$subject['id']?>">
					...
					<?php
					if($subject['branch_id'] == $this->config->item('branch_for_special_feature')){
					?>
					<div class="form-group">
						<label class="col-md-3 control-label">
							<?=translate('subject_category')?> <span class="required">*</span>
						</label>
						<div class="col-md-6">
							<select name="subject_category" id="subject_category" class="form-control">
							    <option value="normal" <?php if($subject['subject_category'] == 'normal'){?> selected <?php }; ?>>Normal</option>
							    <option value="ismalia" <?php if($subject['subject_category'] == 'ismalia'){?> selected <?php } ?>>Ismalia</option>
							</select>
						</div>
					</div>
					<?php
